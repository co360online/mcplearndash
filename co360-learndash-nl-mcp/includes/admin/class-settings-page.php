...
class CO360_LDMCP_Settings_Page {
    ...
    public function register_page(): void {
        add_submenu_page(
            'co360-ldmcp',
            __( 'Ajustes IA', 'co360-learndash-nl-mcp' ),
            __( 'Ajustes IA', 'co360-learndash-nl-mcp' ),
            'manage_options',
    ...
    public function render(): void {
        if ( isset( $_POST['co360_ldmcp_settings_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['co360_ldmcp_settings_nonce'] ) ), 'co360_ldmcp_settings' ) ) {
            ...
            echo '<div class="updated"><p>' . esc_html__( 'Ajustes guardados', 'co360-learndash-nl-mcp' ) . '</p></div>';
        }

        $models = [ 'gpt-4.1', 'gpt-4o', 'gpt-4o-mini', 'custom' ];
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Ajustes de IA (OpenAI)', 'co360-learndash-nl-mcp' ); ?></h1>
            <form method="post">
                ...
                <?php submit_button( __( 'Guardar ajustes', 'co360-learndash-nl-mcp' ) ); ?>
            </form>
        </div>
        <?php
    }
