...
class CO360_LDMCP_Generate_MCP_Page {
    ...
    public function register_page(): void {
        add_submenu_page(
            'co360-ldmcp',
            __( 'Generar MCP', 'co360-learndash-nl-mcp' ),
            __( 'Generar MCP', 'co360-learndash-nl-mcp' ),
            'manage_options',
    ...
    public function render(): void {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Generar MCP con IA', 'co360-learndash-nl-mcp' ); ?></h1>
            <form method="post">
                <?php wp_nonce_field( 'co360_ldmcp_generate', 'co360_ldmcp_generate_nonce' ); ?>
                <p><label><?php esc_html_e( 'Briefing', 'co360-learndash-nl-mcp' ); ?></label><br/>
                    <em><?php esc_html_e( 'Opcional: puedes indicar fechas con ‘Inicio del curso: 2026-03-01’ y ‘Fin del curso: 2026-06-30’ (o DD/MM/YYYY).', 'co360-learndash-nl-mcp' ); ?></em><br/>
                    <textarea name="briefing" rows="5" cols="80"></textarea></p>
                <p><label><?php esc_html_e( 'Tipo de curso', 'co360-learndash-nl-mcp' ); ?></label>
                ...
                <?php submit_button( __( 'Generar MCP con IA', 'co360-learndash-nl-mcp' ) ); ?>
            </form>
            <?php if ( is_array( $date_info ) ) : ?>
                <div class="notice notice-info"><p>
                    <?php
                    if ( null !== $date_info['course_start_ts'] ) {
                        echo esc_html__( 'Fecha inicio detectada: ', 'co360-learndash-nl-mcp' ) . esc_html( wp_date( 'Y-m-d', (int) $date_info['course_start_ts'] ) );
                        echo '<br/>';
                    }
                    if ( null !== $date_info['course_end_ts'] ) {
                        echo esc_html__( 'Fecha fin detectada: ', 'co360-learndash-nl-mcp' ) . esc_html( wp_date( 'Y-m-d', (int) $date_info['course_end_ts'] ) );
                    }
                    if ( null === $date_info['course_start_ts'] && null === $date_info['course_end_ts'] ) {
                        esc_html_e( 'No se han indicado fechas', 'co360-learndash-nl-mcp' );
                    }
                    ?>
                </p></div>
            <?php endif; ?>
            ...
            <?php endif; ?>
            <?php if ( $output ) : ?>
                <h2><?php esc_html_e( 'MCP generado', 'co360-learndash-nl-mcp' ); ?></h2>
