...
class CO360_LDMCP_Templates_Page {
    ...
    public function register_page(): void {
        add_submenu_page(
            'co360-ldmcp',
            __( 'Plantillas MCP', 'co360-learndash-nl-mcp' ),
            __( 'Plantillas MCP', 'co360-learndash-nl-mcp' ),
            'manage_options',
    ...
    public function render(): void {
        if ( isset( $_POST['co360_ldmcp_template_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['co360_ldmcp_template_nonce'] ) ), 'co360_ldmcp_template' ) ) {
            ...
            if ( $name && $template ) {
                ...
                echo '<div class="updated"><p>' . esc_html__( 'Plantilla guardada', 'co360-learndash-nl-mcp' ) . '</p></div>';
            }
        }
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Plantillas MCP', 'co360-learndash-nl-mcp' ); ?></h1>
            <form method="post">
                <?php wp_nonce_field( 'co360_ldmcp_template', 'co360_ldmcp_template_nonce' ); ?>
                <p><label><?php esc_html_e( 'Nombre', 'co360-learndash-nl-mcp' ); ?></label><br/>
                    <input type="text" name="template_name" /></p>
                <p><label><?php esc_html_e( 'Payload parcial', 'co360-learndash-nl-mcp' ); ?></label><br/>
                    <textarea name="template_payload" rows="8" cols="100"></textarea></p>
                <?php submit_button( __( 'Guardar plantilla', 'co360-learndash-nl-mcp' ) ); ?>
            </form>
            <h2><?php esc_html_e( 'Plantillas guardadas', 'co360-learndash-nl-mcp' ); ?></h2>
            <?php if ( $templates ) : ?>
                <ul>
            ...
            <?php else : ?>
                <p><?php esc_html_e( 'Sin plantillas guardadas aún.', 'co360-learndash-nl-mcp' ); ?></p>
            <?php endif; ?>
        </div>
        <?php
    }
