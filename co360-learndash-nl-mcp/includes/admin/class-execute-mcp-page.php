...
class CO360_LDMCP_Execute_MCP_Page {
    ...
    public function register_page(): void {
        add_submenu_page(
            'co360-ldmcp',
            __( 'Ejecutar MCP', 'co360-learndash-nl-mcp' ),
            __( 'Ejecutar MCP', 'co360-learndash-nl-mcp' ),
            'manage_options',
            'co360-ldmcp-execute',
            [ $this, 'render' ]
    ...
    public function render(): void {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e( 'Ejecutar MCP', 'co360-learndash-nl-mcp' ); ?></h1>
            <form method="post">
                <?php wp_nonce_field( 'co360_ldmcp_execute', 'co360_ldmcp_execute_nonce' ); ?>
                <p><label><?php esc_html_e( 'Pega el MCP Payload', 'co360-learndash-nl-mcp' ); ?></label><br/>
                    <textarea name="payload" rows="12" cols="100"></textarea></p>
                <?php submit_button( __( 'Validar y ejecutar', 'co360-learndash-nl-mcp' ) ); ?>
            </form>
            <?php if ( $result ) : ?>
                <h2><?php esc_html_e( 'Resultado', 'co360-learndash-nl-mcp' ); ?></h2>
